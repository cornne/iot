...
// Runtime state
bool isPowerOn = true;
bool hasInitialPowerChecked = false;
...
void loop() {
  // 1. KIỂM TRA TRẠNG THÁI CÔNG TẮC NGUỒN (GPIO 5)
  // HIGH = BẬT MẠCH (ON) | LOW = TẮT MẠCH (OFF/SLEEP)
  bool currentSwitchState = (digitalRead(POWER_SWITCH_PIN) == HIGH);

  if (!hasInitialPowerChecked || currentSwitchState != isPowerOn) {
    hasInitialPowerChecked = true;
    isPowerOn = currentSwitchState;

    if (!isPowerOn) {
      // KHI GẠT CÔNG TẮC SANG OFF (TẮT NGUỒN):
      Serial.println("\n[POWER] >>> CÔNG TẮC GẠT: TẮT NGUỒN MẠCH (SYSTEM SLEEP / OFF) <<<");
      // Dập tắt toàn bộ đèn LED & Còi hú
      digitalWrite(LED_ALERT_RED_PIN, LOW);
      digitalWrite(LED_SAFE_GREEN_PIN, LOW);
      noTone(BUZZER_PIN);
      isAllergenAlertActive = false;
      ...
      // Hiển thị tắt nguồn rồi tắt đèn nền LCD
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("POWER: [OFF]");
      lcd.setCursor(0, 1);
      ...
      // KHI GẠT CÔNG TẮC SANG ON (BẬT NGUỒN):
      Serial.println("\n[POWER] >>> CÔNG TẮC GẠT: BẬT NGUỒN MẠCH (SYSTEM ON) <<<");
      lcd.backlight();
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Sadie's Link ESP");
  ...
  // Nếu công tắc đang ở vị trí TẮT (OFF), tạm dừng mọi tác vụ
  if (!isPowerOn) {
    delay(100);
    return;
  }

  // 2. KHI MẠCH ĐANG BẬT (ON): TIẾP TỤC VẬN HÀNH BÌNH THƯỜNG
  if (!client.connected()) {
    reconnectMqtt();
  }
  client.loop();
